  ...
  return (
    <div class="flex flex-col gap-2 w-full">
      <Show when={props.options.isSearchable}>
        <div class="flex items-end probot-input w-full">
          <SearchInput
            ref={inputRef}
            onInput={filterItems}
        ...
        <For each={filteredItems()}>
          {(item, index) => (
            <button
              ...
              data-itemid={item.id}
              class={
                'flex flex-col probot-picture-button focus:outline-none filter hover:brightness-90 active:brightness-75 justify-between  ' +
